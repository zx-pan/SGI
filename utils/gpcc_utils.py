...
def read_ply_geo_bin(ply_path: str) -> np.ndarray:
    ...
    means = np.stack([ply_data.data[name] for name in ['x', 'y']], axis=1)  # shape (N, 2)
    return means
...
def decompress_gaussian_params(
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with open(bin_path, 'rb') as f:
            ...
            means_min, means_max = head_info[:2], head_info[2:]  # 3 or 2? not sure yet
...
def compress_gpcc(x: torch.Tensor, gpcc_codec_path: str='tmc3') -> bytes:
    ...
    Compress geometry point cloud by GPCC codec.
    """
    assert len(x.shape) == 2 and x.shape[1] == 2, f'Input data must be a 2D point cloud, but got {x.shape}.'

    # add a z dim temporarily for GPCC codec running successfully
