...
class _grid_encode(Function):
    ...
    def forward(ctx, inputs, embeddings, offsets_list, resolutions_list, calc_grad_inputs=False, min_level_id=None, n_levels_calc=1, binary_vxl=None, PV=0):
        ...
        N, num_dim = inputs.shape  # batch size, coordinate dimension (e.g., N_rays, 3)
        n_levels = offsets_list.shape[0] - 1  # number of hash levels (e.g., 16)
        n_features = embeddings.shape[1]  # embedding channels per level (e.g., 2)
        ...
        outputs = torch.empty(n_levels_calc, N, n_features, device=inputs.device, dtype=embeddings.dtype)  # Allocate CUDA output buffer.
        # outputs layout: [num_levels, N_rays, channels]
