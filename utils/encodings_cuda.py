...
# Default chunk size for arithmetic coding. Can be overridden via set_chunk_size_cuda().
chunk_size_cuda = 100000


def set_chunk_size_cuda(new_size: int):
    """Set the global chunk size for arithmetic encoding/decoding."""
    global chunk_size_cuda
    chunk_size_cuda = new_size


def get_chunk_size_cuda() -> int:
    """Get the current global chunk size for arithmetic encoding/decoding."""
    return chunk_size_cuda
...
def decoder_gaussian_with_stream(mean, scale, Q, file_name, stream=None):
    """
    Decoder that can run on a specific CUDA stream for parallel execution.
    """
    assert file_name.endswith('.b')
    ...
    # File I/O (CPU bound)
    with open(file_name, 'rb') as fin:
        min_value_np = np.frombuffer(fin.read(4), dtype=np.float32).copy()
        max_value_np = np.frombuffer(fin.read(4), dtype=np.float32).copy()
        len_cnt_bytes = np.frombuffer(fin.read(4), dtype=np.int32)[0]
        cnt_np = np.frombuffer(fin.read(len_cnt_bytes), dtype=np.int32).copy()
        byte_stream_np = np.frombuffer(fin.read(), dtype=np.uint8).copy()

    # GPU operations on specified stream
    if stream is not None:
        with torch.cuda.stream(stream):
            min_value = torch.tensor(min_value_np, device="cuda")
            max_value = torch.tensor(max_value_np, device="cuda")
            cnt_torch = torch.tensor(cnt_np, device="cuda")
            byte_stream_torch = torch.tensor(byte_stream_np, device="cuda")
            
            lower = arithmetic.calculate_cdf(mean, scale, Q, min_value, max_value)
            sym_out = arithmetic.arithmetic_decode(
                lower, byte_stream_torch, cnt_torch, chunk_size_cuda,
                int(lower.shape[0]), int(lower.shape[1])
            ).to(mean.device).to(torch.float32)
            x = (sym_out + min_value) * Q
    else:
        min_value = torch.tensor(min_value_np, device="cuda")
        max_value = torch.tensor(max_value_np, device="cuda")
        cnt_torch = torch.tensor(cnt_np, device="cuda")
...
def decoder_gaussian_chunk_with_stream(mean, scale, Q, file_name='tmp.b', chunk_size=1000_0000, stream=None):
    """Version of decoder_gaussian_chunk that uses a specific CUDA stream."""
    assert file_name.endswith('.b')
    mean_view = mean.view(-1)
    scale_view = scale.view(-1)
    N = mean_view.shape[0]
    ...
    x_c_list = []
    for c in range(chunks):
        x_c = decoder_gaussian_with_stream(
            mean=mean_view[c*chunk_size:c*chunk_size + chunk_size],
            scale=scale_view[c*chunk_size:c*chunk_size + chunk_size],
            Q=Q_view[c*chunk_size:c*chunk_size + chunk_size] if Is_Q_tensor else Q,
            file_name=file_name.replace('.b', f'_{str(c)}.b'),
...
def parallel_decode_features(decode_args_list, num_streams=5):
    """
    Decode multiple features in parallel using CUDA streams.
    
    Args:
        decode_args_list: List of tuples (mean, scale, Q, file_name, chunk_size)
    ...
    n_tasks = len(decode_args_list)
    streams = [torch.cuda.Stream() for _ in range(min(num_streams, n_tasks))]
    results = [None] * n_tasks
    
    def decode_task(idx, args, stream):
        mean, scale, Q, file_name, chunk_size = args
        result = decoder_gaussian_chunk_with_stream(mean, scale, Q, file_name, chunk_size, stream)
        return idx, result
    
    # Use ThreadPoolExecutor for parallel file I/O
    with ThreadPoolExecutor(max_workers=num_streams) as executor:
        futures = []
        for i, args in enumerate(decode_args_list):
            stream = streams[i % len(streams)]
            futures.append(executor.submit(decode_task, i, args, stream))
...
def parallel_decode_all(decode_args_list, num_streams=7):
    """
    Decode ALL operations in parallel using a 2-phase approach:
    
    Phase 1: Pre-load all files in parallel (I/O bound, use threads)
    Phase 2: Queue all GPU operations to streams (GPU bound, no thread overhead)
    
    Args:
        decode_args_list: List of tuples (mean, scale, Q, file_name, chunk_size, op_type, op_idx)
        num_streams: Number of CUDA streams to use
    
    Returns:
        List of decoded tensors in same order as input
    """
    from concurrent.futures import ThreadPoolExecutor
    
    chunk_size_cuda = get_chunk_size_cuda()
    n_tasks = len(decode_args_list)
    
    # ========== Phase 1: Collect all file paths and pre-load in parallel ==========
    # Build list of all chunk files needed
    file_info_list = []  # (task_idx, chunk_idx, file_path, mean_slice, scale_slice, Q_slice)
    
    for task_idx, args in enumerate(decode_args_list):
        mean, scale, Q, file_name, chunk_size, op_type, op_idx = args
        mean_view = mean.view(-1)
        scale_view = scale.view(-1)
        N = mean_view.shape[0]
        chunks = int(np.ceil(N / chunk_size))
        ...
        for c in range(chunks):
            chunk_file = file_name.replace('.b', f'_{str(c)}.b')
            mean_slice = mean_view[c*chunk_size:c*chunk_size + chunk_size]
            scale_slice = scale_view[c*chunk_size:c*chunk_size + chunk_size]
            Q_slice = Q_view[c*chunk_size:c*chunk_size + chunk_size] if Is_Q_tensor else Q
            file_info_list.append((task_idx, c, chunk_file, mean_slice, scale_slice, Q_slice))
    ...
    def load_file(file_path):
        with open(file_path, 'rb') as fin:
            ...
            cnt_np = np.frombuffer(fin.read(len_cnt_bytes), dtype=np.int32).copy()
            byte_stream_np = np.frombuffer(fin.read(), dtype=np.uint8).copy()
        return file_path, (min_value_np, max_value_np, cnt_np, byte_stream_np)
    
    file_cache = {}
    unique_files = list(set(info[2] for info in file_info_list))
    with ThreadPoolExecutor(max_workers=min(len(unique_files), 16)) as executor:
        futures = [executor.submit(load_file, fp) for fp in unique_files]
        for future in futures:
            path, data = future.result()
            file_cache[path] = data
    
    # ========== Phase 2: Queue all GPU operations to streams ==========
    # Group by task_idx to collect chunks for each task
    task_chunks = {i: [] for i in range(n_tasks)}
    for task_idx, chunk_idx, file_path, mean_slice, scale_slice, Q_slice in file_info_list:
        task_chunks[task_idx].append((chunk_idx, file_path, mean_slice, scale_slice, Q_slice))
    
    # Create streams
    streams = [torch.cuda.Stream() for _ in range(min(num_streams, n_tasks))]
    results = [None] * n_tasks
    
    # Queue all GPU operations - this is the key: simple stream blocks, no thread overhead
    for task_idx in range(n_tasks):
        stream = streams[task_idx % len(streams)]
        chunks_data = sorted(task_chunks[task_idx], key=lambda x: x[0])  # Sort by chunk_idx
        
        x_c_list = []
        with torch.cuda.stream(stream):
            for chunk_idx, file_path, mean_slice, scale_slice, Q_slice in chunks_data:
                cached = file_cache[file_path]
                min_value_np, max_value_np, cnt_np, byte_stream_np = cached
                
                # Prepare Q tensor if needed
                if not isinstance(Q_slice, torch.Tensor):
                    Q_tensor = torch.tensor([Q_slice], dtype=mean_slice.dtype, device=mean_slice.device).repeat(mean_slice.shape[0])
                else:
                    Q_tensor = Q_slice
                
                # Transfer to GPU and decode
                min_value = torch.tensor(min_value_np, device="cuda")
                max_value = torch.tensor(max_value_np, device="cuda")
                cnt_torch = torch.tensor(cnt_np, device="cuda")
                byte_stream_torch = torch.tensor(byte_stream_np, device="cuda")
                
                lower = arithmetic.calculate_cdf(mean_slice, scale_slice, Q_tensor, min_value, max_value)
                sym_out = arithmetic.arithmetic_decode(
                    lower, byte_stream_torch, cnt_torch, chunk_size_cuda,
                    int(lower.shape[0]), int(lower.shape[1])
                ).to(mean_slice.device).to(torch.float32)
                x_c = (sym_out + min_value) * Q_tensor
                x_c_list.append(x_c)
        
        # Concatenate chunks for this task (still on the stream)
        args = decode_args_list[task_idx]
        mean = args[0]
        with torch.cuda.stream(stream):
            results[task_idx] = torch.cat(x_c_list, dim=0).type_as(mean)
